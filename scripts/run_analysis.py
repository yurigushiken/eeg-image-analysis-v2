...
from eeg.io import discover_epoch_files, read_epochs, apply_montage
...
def main() -> int:
    ...
    if len(fif_files) == 0:
        ...
        # Prepare per-set subject evokeds
        ...
        for fif in fif_files:
            ...
            for item in sets:
                ...
                if getattr(epochs, "metadata", None) is not None and "Condition" in epochs.metadata.columns:
                    ...
                    if sub is None:
                        continue
                if len(sub) < int(cfg.selection.get("min_epochs_per_set", 1)):
                    continue
                evk = sub.average()
    ...
    write_analysis_page(page_path, title=os.path.splitext(os.path.basename(page_path))[0], figure_paths=saved_figs)
    ...
    if tables_dir:
        ...
        metrics = {
            ...
            "largest_png_bytes": (max(sizes.values()) if sizes else 0),
            "total_png_bytes": int(sum(sizes.values())) if sizes else 0,
            "figures": sizes,
        }

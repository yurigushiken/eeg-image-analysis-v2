...
def test_compute_peak_to_peak_metrics_synthetic_signal():
    """
    Fail-first: expects a helper to compute P1↔N1 peak-to-peak metrics
    from a single ROI waveform using N1 ROI only.

    The function under test should live in eeg.measures:
      compute_peak_to_peak_metrics(signal, times_ms, p1_window_ms, n1_window_ms,
                                   p_polarity='positive', n_polarity='negative')
    and return a dict with keys: 'p1_amp', 'n1_amp', 'p2p_amp', 'p1_lat_ms', 'n1_lat_ms'.
    """
    ...
    # Import here to trigger fail-first until implementation exists
    from eeg.measures import compute_peak_to_peak_metrics  # type: ignore
